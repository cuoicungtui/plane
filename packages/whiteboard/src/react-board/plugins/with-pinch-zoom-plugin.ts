/* oxlint-disable no-shadow -- vendored from Drawnix, kept close to upstream */
import {
  BoardTransforms,
...
export const withPinchZoom = (board: PlaitBoard) => {
  ...
  board.touchEnd = (event: TouchEvent) => {
    const index = pointerRecords.findIndex((r) => r.pointerId === event.changedTouches[0].identifier);
    if (index !== -1) {
