...
import { BoardContext, type BoardContextValue } from "./hooks/use-board";
...
export const Wrapper: React.FC<WrapperProps> = ({
  ...
  }, [board, listRender, onChange, onSelectionChange, onThemeChange, onValueChange, onViewportChange]);
  ...
  let board = withRelatedFragment(
    withHotkey(
      withHandPointer(
        withHistory(
          withSelection(
            withMoving(withBoard(withI18n(withOptions(withReact(withImage(withText(createBoard(value, options))))))))
          )
        )
      )
    )
