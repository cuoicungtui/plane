...
// Excalidraw renders real DOM nodes (a <textarea> overlay for the text tool,
// native mouse-drag drawing on its canvas) inside this NodeView, which sits
// inside ProseMirror's contentEditable root. React's synthetic event system
...
// (the text tool's textarea still lost focus and got deleted with that
// approach in place). ProseMirror sees the events, creates a NodeSelection on
// this atom node, and steals DOM focus back onto its contentEditable root:
// for the text tool this happens right as Excalidraw creates its editing
// textarea, and losing focus before any character is typed makes Excalidraw
// treat it as an abandoned empty text box and delete it; for keystrokes,
// ProseMirror's default typing-over-a-selection behavior deletes the whole
// embed and inserts the character in its place.
//
...
// node's DOM, without touching the real DOM event at all — so Excalidraw
// still receives and bubbles every event completely normally.
const stopEvent = () => true;

// `stopEvent` alone isn't enough: activating the text tool inserts a new
// <textarea> DOM node inside this NodeView, which is a childList mutation.
...
// right back from the textarea Excalidraw just created and focused. This is
